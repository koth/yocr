...
void SquishedDawg::read_squished_dawg(FILE *file,
  ...
  inT16 magic;
  size_t r = fread(&magic, sizeof(inT16), 1, file);
  ASSERT_HOST(r == 1);
  bool swap = (magic != kDawgMagicNumber);

  int unicharset_size;
  r = fread(&unicharset_size, sizeof(inT32), 1, file);
  ASSERT_HOST(r == 1);
  r = fread(&num_edges_, sizeof(inT32), 1, file);
  ASSERT_HOST(r == 1);
  ...
  r = fread(&edges_[0], sizeof(EDGE_RECORD), num_edges_, file);
  ASSERT_HOST(r == num_edges_);
  EDGE_REF edge;
  if (swap) {
